...
const copyProjectTemplate = async (projectName, language, template) => {
	let fullPath = `${path.dirname(require.main.filename)}/templates/projects/${language}/${template}/`
...
const copyCIConfigTemplate = async (projectName, template) => {
	let fullPath = `${path.dirname(require.main.filename)}/templates/ci-config/.${template}.yml`
