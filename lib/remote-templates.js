...
const unzipProjectArchive = async (repo, dir) => {
	try {
		...
		throw new Error(`Problem unzipping archived template...(${dir}/${repo}.zip)\n${err}`)
	}
}

const moveRepoContentsToProjectFolder = async (repo, dir, ref) => {
	let contents = await fs.readdir(`${dir}/${repo}-${ref}/`)

	// If contents not empty, move files to dir
	if (contents.length > 0) {
		for await (const file of contents) {
			let fileStat = await fs.pathExists(`${dir}/${file}`) // Check if destination file already exists (do not overwrite)
			if (!fileStat) await fs.move(`${dir}/${repo}-${ref}/${file}`, `${dir}/${file}`, {
				overwrite: false
			})
		}

		// Now delete root directory of repo
		await fs.remove(`${dir}/${repo}-${ref}/`)
	} else {
...
const getRemoteTemplate = async (user, repo, ref, dir) => {
	try {
		...
		await moveRepoContentsToProjectFolder(repo, dir, ref)
