...
const askProjectDetails = () => {
	...
	let nodejsTemplates
	try {
		pythonTemplates = files.getProjectTemplates(langChoice[langChoice.indexOf('python')])
		nodejsTemplates = files.getProjectTemplates(langChoice[langChoice.indexOf('nodejs')])
	} catch (err) {
		console.log(chalk.yellow("WARNING: ") + chalk.white(err + " (we will continue without cloud templates) "))
		useCloud = false // Disable cloud usage on failed attempt
	}
	const typeChoice = {
	...
	const questions = [
		...
		},
		{
			...
			choices : langChoice,
			when    : (answers) => useCloud && answers['use_template'] && !answers['use_external']
		},
		{
			name    : 'project_type',
			type    : 'list',
			message : 'What type of project are you looking to create? ',
			choices : (answers) => useCloud ? typeChoice[answers['project_lang']] : [],
			when    : (answers) => useCloud && answers['use_template'] && !answers['use_external']
		},
		{
			name    : 'include_readme',
