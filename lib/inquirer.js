const files = require('./files')
const path = require('path')
const inquirer = require('inquirer')
const touch = require('touch')

const askGithubToken = () => {
	const questions = [
		{
			name     : 'token',
			type     : 'password',
			message  : 'Enter your Github OAuth Token: ',
			validate : (value) => {
				if (value.length) {
					return true
				} else {
					return 'Please enter your Oauth Token.'
				}
			}
		}
	]
	return inquirer.prompt(questions)
}

const askGitIgnore = async (project_name) => {
	let fileNames = []
	if (files.directoryExists(`${files.getCurrentDirectory()}/${project_name}`)) {
		let res = await files.getFileFoldersInDir('.')
		for (let i = 0; i < res.length; i++) {
			res[i] = { name: res[i] }
		}
		fileNames = res
	} else {
		touch('.gitignore')
	}

	const question = [
		{
			name    : 'gitignore_files',
			type    : 'checkbox',
			choices : fileNames
		}
	]

	if (fileNames.length > 0) return inquirer.prompt(question)
	return null
}

const askProjectDetails = () => {
	const githubFeatures = [
		{ name: 'has_issues', checked: true },
		{ name: 'has_projects', checked: true },
		{ name: 'has_wiki', checked: true }
	]
	const langChoice = [
		'python',
		'nodejs'
	]
	const typeChoice = {
		python : [
			'basic',
			'flask'
		],
		nodejs : [
			'basic',
			'express.js',
			'create-react-app'
		]
	}
	const readme_styles = [
		'standard',
		'minimal',
		'hackathon',
		'bot'
	]
	const cicdProviders = [
		'travis-ci'
	]

	const questions = [
		{
			name     : 'project_name',
			type     : 'input',
			message  : `🤔 What do you want to call this project? `,
			validate : (value) => {
				if (value.length) {
					return true
				} else {
					return 'Enter a valid project name.'
				}
			},
			default  : () => {
				return files.getCurrentDirectory()
			}
		},
		{
			name     : 'project_description',
			type     : 'input',
			message  : 'Enter a short description of your project',
			validate : (value) => {
				if (value.length) return true
				return 'Please enter something to describe your project.'
			},
			default  : () => 'A project made using gitBuilder.io'
		},
		{
			name    : 'project_visibility',
			type    : 'list',
			message : 'Is this a Public or Private project? ',
			choices : [
				'public',
				'private'
			]
		},
		{
			name    : 'project_homepage',
			type    : 'input',
			message : 'Enter your project homepage: ',
			default : () => 'https://example.com'
		},
		{
			name     : 'project_features',
			type     : 'checkbox',
			message  : 'Select all GitHub features you would like to use for this project: ',
			validate : function(answer) {
				if (answer.length < 1) {
					return 'You must choose at least one feature to include.'
				}

				return true
			},
			choices  : githubFeatures
		},
		{
			name    : 'project_lang',
			type    : 'list',
			message : 'What language are you planning to (primarily) use? ',
			choices : langChoice
		},
		{
			name    : 'project_type',
			type    : 'list',
			message : 'What type of project are you looking to create?',
			choices : (answers) => typeChoice[answers['project_lang']]
		},
		{
			name    : 'include_readme',
			type    : 'confirm',
			message : 'Would you like to include a README file?'
		},
		{
			name    : 'readme_style',
			type    : 'list',
			message : 'Select a style for your README: ',
			choices : readme_styles,
			when    : (answers) => answers['include_readme']
		},
		{
			name    : 'include_cicd',
			type    : 'confirm',
			message : 'Would you like to use CI/CD? '
		},
		{
			name    : 'cicd_provider',
			type    : 'list',
			message : 'What CICD Provider do you want to use? ',
			choices : cicdProviders,
			when    : (answers) => answers['include_cicd']
		},
		{
			name    : 'repo_tags',
			type    : 'input',
			message : 'Enter some tags for your repo (sep. by comma or ENTER to SKIP): ',
			default : () => ''
		}
	]

	return inquirer.prompt(questions)
}

module.exports = {
	askGithubToken,
	askProjectDetails,
	askGitIgnore
}
