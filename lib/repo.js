...
const initLocalRepo = async (projectName) => {
	try {
		...
		const { stderr } = await exec(`git init ${files.getProjectDirectoryIfExists(projectName).replace(/(\s+)/g, '\\$1')}`)
		return !stderr
	} catch (err) {
		throw new Error(err)
	}
	...
	try {
		if (remoteSource !== null) {
			const { stderr } = await exec(
				`cd ${files.getProjectDirectoryIfExists(projectName).replace(/(\s+)/g, '\\$1')} && git remote add origin ${remoteSource}.git`
			)
			return !stderr
		} else {
...
const publishProjectContent = async (projectName) => {
	try {
		const { stderr } = await exec(`cd ${files.getProjectDirectoryIfExists(projectName).replace(/(\s+)/g, '\\$1')} && git add -A`)
		if (!stderr) {
			const { stderr } = await exec(
				`cd ${files.getProjectDirectoryIfExists(
					projectName
				).replace(/(\s+)/g, '\\$1')} && git commit -m "gitBuilder: autogenerated project"`
			)
			if (!stderr) {
				const { stderr } = await exec(
					`cd ${files.getProjectDirectoryIfExists(projectName).replace(/(\s+)/g, '\\$1')} && git push --set-upstream origin master`
				)
				return !stderr
			}
		}
